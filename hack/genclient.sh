...
"$GOBIN"/client-gen \
  ...
  --output-pkg github.com/jaroslav-viravec/valkey-operator/pkg/client/clientset \
  --output-dir "$TEMPDIR"/pkg/client/clientset \
  --plural-exceptions Valkey:valkeies

"$GOBIN"/lister-gen \
  --go-header-file "$BASEDIR"/hack/boilerplate.go.txt \
  --output-pkg github.com/jaroslav-viravec/valkey-operator/pkg/client/listers \
  --output-dir "$TEMPDIR"/pkg/client/listers \
  --plural-exceptions Valkey:valkeies \
  github.com/jaroslav-viravec/valkey-operator/tmp/gen/apis/cache.cs.sap.com/v1alpha1

"$GOBIN"/informer-gen \
  --versioned-clientset-package github.com/jaroslav-viravec/valkey-operator/pkg/client/clientset/versioned \
  --listers-package github.com/jaroslav-viravec/valkey-operator/pkg/client/listers \
  --go-header-file "$BASEDIR"/hack/boilerplate.go.txt \
  --output-pkg github.com/jaroslav-viravec/valkey-operator/pkg/client/informers \
